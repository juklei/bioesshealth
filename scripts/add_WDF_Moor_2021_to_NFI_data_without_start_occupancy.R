...
## 4.1. Predict occupancy, colonisation, and extinction ------------------------
##      Instead of defingin start occupancy with an occupancy model, we run 
##      colonisation extinction dynamics N times until stabilisatoin:
...
pred_dyn_init <- function(x, N){
  ...
  for(i in 2:N){
    dyn_M[i, ] <- as.matrix((1 - dyn_M[i-1, ]) * x[x$probability == "col", ..spec] +
                              dyn_M[i-1, ] * (1 - x[x$probability == "ext", ..spec]))
  }
  
  out <- cbind(1:i, dyn_M)
  colnames(out) <- c("dummy_period", spec)
...
pred_dyn <- function(x){
  dyn_M[1, ] <- as.matrix((1 - dcip_fin[dcip_fin$Description %in% x$NFI, ..spec])*
                          x[x$probability == "col" & x$period == 1, ..spec] +
                            dcip_fin[dcip_fin$Description %in% x$NFI, ..spec]*
